...
description: Real-time research lookup using Claude Code’s built-in web tools (WebSearch/WebFetch) or provided URLs to gather recent papers, stats, protocols, and cite sources. No external API keys required.
allowed-tools: [Read, Write, Edit, Bash, WebSearch, WebFetch]
---

# Research Information Lookup (Claude-native)

## Purpose
Fetch current research, papers, stats, and protocols using Claude Code’s native browsing tools (WebSearch/WebFetch) or user-provided links. Produce concise syntheses with source citations.

## When to Use
- Need fresh or citation-backed info (papers, trials, stats, protocols, standards).
- User asks for “recent”, “latest”, “current”, or wants citations/links.
- You have URLs to inspect, or browsing tools are available.

## Protocol (no API keys)
1) **Clarify** the research question and date sensitivity.  
2) **Search**:  
   - If URLs are provided: fetch and read them.  
   - Else use `WebSearch` with 2–4 focused queries; pick 3–6 best hits.  
3) **Read** key sources with `WebFetch`; extract title, authors/org, date, and key findings.  
4) **Synthesize**: Summarize converging findings; note disagreements; include 3–6 citations with links.  
5) **Validate**: Quote numbers/dates; avoid speculation; mark gaps clearly.  
6) **Deliver**: Provide a brief answer plus a reference list (title, source, year, URL).  

## Output Format
- Short answer with numbered inline citations.  
- “Top sources” list with title – source – year – URL.  
- Optional next steps: additional queries, data sources, or what to ask the user for.

## Guardrails
- No fabricated citations. Only cite fetched or user-provided sources.  
- If browsing is unavailable, ask the user for links or to enable search; do not invent results.  
- Prefer recent (≤2 years) when recency matters; otherwise include seminal sources.  

...
class AgentMonitor:
    ...
    def is_claude_ready(self, content: str) -> bool:
        ...
        ready_indicators = [
            ...
            ("> Try" in content),  # Simple fallback
            bool(re.search(r">\\s*Try", content)),  # Handles non-breaking or irregular spaces
...
class ClaudeAgentFarm:
    ...
    def _is_content_ready(self, content: str) -> bool:
        """Local readiness check (mirrors AgentMonitor.is_claude_ready but doesn't require monitor)"""
        ready_indicators = [
            "Welcome to Claude Code!" in content,
            ("│ > Try" in content),
            ("> Try" in content),
            bool(re.search(r">\\s*Try", content)),
    ...
    def start_agent(self, agent_id: int, restart: bool = False) -> None:
        ...
        for attempt in range(max_retries):
            ...
            if not content or len(content.strip()) < 10:
                ...
                ready = (self.monitor and self.monitor.is_claude_ready(content)) or self._is_content_ready(content)
                if attempt == 0:
                    console.print(f"[dim]Agent {agent_id:02d}: ready_check={ready} (len={len(content)})[/dim]")
                if ready or len(content.strip()) > 100:
                    ...
                    # Final readiness check before declaring failure
                    final_content = tmux_capture(pane_target)
                    if (self.monitor and self.monitor.is_claude_ready(final_content)) or self._is_content_ready(final_content):
                        claude_started_successfully = True
                    else:
                        console.print(
